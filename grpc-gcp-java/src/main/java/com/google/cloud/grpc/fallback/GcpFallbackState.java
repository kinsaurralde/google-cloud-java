...
public class GcpFallbackState {
  ...
  public synchronized ScheduledExecutorService getOrCreateExecutorService(
    ...
    if (externalExec != null) {
      ...
      this.execService =
          Executors.newScheduledThreadPool(3, GcpThreadFactory.newThreadFactory("gcp-fallback-state-%d"));
  ...
  /**
   * Starts the periodic error rate evaluation loop exactly once across all channels sharing this state.
   *
  ...
  /** Shuts down the state immediately, cancelling evaluation and terminating internal executor if owned. */
